...
token_rec scanner_class::check_token(const char * c_str)
{
	...
	err_token.type = token_type::ERRTOKEN;
	return err_token;
}
...
token_rec scanner_class::get_token(void)
{
	...
	for (;;)	// 过滤源程序中的空格、TAB、回车等，遇到文件结束符返回空记号
	{
		...
		if (next_char == EOF)
		{
			token.type = token_type::NONTOKEN;
			return token;
		}
		if (next_char == '\n')	line_no++;
	...
	// 则先加入到记号的字符缓冲区中
	if (isalpha(next_char) || next_char == '_')		// 若char是A-Za-z，则它一定是函数、关键字、PI、E等
	{
		for (;;)
		{
			next_char = get_char();
			if (isalnum(next_char) || next_char == '_')	add_in_token_str(next_char);
			else				break;
		}
		back_char(next_char);
	...
	else	// 不是字母和数字，则一定是符号
	{
		switch (next_char)
		{
		case ';': token.type = token_type::SEMICO; break;
		case '(': token.type = token_type::L_BRACKET; break;
		case ')': token.type = token_type::R_BRACKET; break;
		case ',': token.type = token_type::COMMA; break;
		case '+': token.type = token_type::PLUS; break;
		case '-':
			next_char = get_char();
			if (next_char == '-')
			{
		...
		case '*':
			...
			break;
		default:
			token.type = token_type::ERRTOKEN;
		} // end of switch
